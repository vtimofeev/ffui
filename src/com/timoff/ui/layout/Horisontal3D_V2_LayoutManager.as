...
public class Horisontal3D_V2_LayoutManager {
    ...
    public static function arrange(container:FFComponent, childs:Array, apply:Boolean):Array {
        ...
        for each (child in childs) {
            ...
            if (apply) {
                ...
                var rotationY = getRotationYFactor(i, activeIndex);
                ...
                childX += getXOffset(i, activeIndex);
    ...
    private static function getXOffset(i:int, activeIndex:int):Number {
        if ( i > 3) return (i-3)*100;
        return 0;
    }
    ...
    public static function getScaleFactor(current:int, active:int):Number {
        return .8 + .3 / getDistanceBetweenIndexes(current, active);
    }
    ...
    public static function getRotationYFactor(current:int, active:int):Number {
        ...
        trace("dest is " + destIndex + " , " + current);
        return   35 * (current - active);//?(2 + (destIndex-2)/3):destIndex;//(destIndex>2?(2 + (destIndex-2)/3):destIndex) ;
